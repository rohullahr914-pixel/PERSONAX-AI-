...
export async function GET() {
  try {
    ...
    return NextResponse.json({ profile: { displayName: row?.display_name ?? user.name, bio: row?.bio ?? "", avatarDataUrl: row?.avatar_data_url ?? undefined, profileVisibility: row?.profile_visibility ?? "Public", quote: row?.quote ?? undefined, quoteVisibility: row?.quote_visibility ?? "Public", favoritePersonaSlugs: favorites.rows.map((item) => item.persona_slug), likedMessages: messages.rows.map((item) => ({ id: item.id, personaName: item.persona_name, personaSlug: item.persona_slug ?? undefined, content: item.content, savedAt: item.saved_at.toISOString() })) } });
  } catch (error) {
...
export async function PATCH(request: Request) {
  try {
    ...
    await query(`INSERT INTO profiles (user_id, display_name, bio, avatar_data_url, profile_visibility) VALUES ($1, $2, $3, $4, COALESCE($5, 'Public'))
      ON CONFLICT (user_id) DO UPDATE SET display_name = COALESCE($2, profiles.display_name), bio = COALESCE($3, profiles.bio), avatar_data_url = CASE WHEN $6 THEN $4 ELSE profiles.avatar_data_url END, profile_visibility = COALESCE($5, profiles.profile_visibility), updated_at = now()`,
      [user.id, displayName, bio, avatar ?? null, visibility, avatar !== undefined]);
