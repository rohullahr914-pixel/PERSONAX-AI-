...
export default function DiscoverPage() {
  ...
  const filteredPersonas = useMemo(() => {
    ...
    return matches.filter((persona) => persona.category === activeFilter);
  }, [activeFilter, query]);
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10 text-white sm:px-6 lg:px-8">
      <div className="relative mb-8 overflow-hidden rounded-[30px] border border-cyan-400/15 bg-slate-950/75 p-5 shadow-[0_0_35px_rgba(34,211,238,0.08)] backdrop-blur-xl">
        <img src="/brand/persona-portraits.png" alt="PersonaX AI historical and expert personas" className="pointer-events-none absolute right-0 top-0 h-full w-1/2 object-contain object-right opacity-20" />
        ...
        <div className="relative z-10 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.25em] text-cyan-300">Discover</p>
      ...
      <div className="mb-8 flex flex-wrap gap-2 text-sm text-slate-200">
          ...
          <button
            ...
            className={`rounded-full border px-3 py-2 transition ${
              activeFilter === filter
                ...
                : "border-white/10 bg-white/3 hover:border-cyan-400/40 hover:bg-cyan-500/5"
            }`}
          >
            {filter}
          </button>
      ...
      <div className="mb-5 flex items-center justify-between text-sm text-slate-300">
        ...
        <Link href="/chat/multi" className="font-medium text-cyan-300 transition hover:text-cyan-200">Open multi-persona chat →</Link>
      </div>
